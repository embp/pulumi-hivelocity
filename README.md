...
1. Run the following command to update files to use the name of your provider (third-party: use your GitHub organization/username):
   ...
   - find and replace all instances of the boilerplate `hivelocity` with the `NAME` of your provider.
   - find and replace all instances of the boilerplate `embp` with the `ORG` of your provider.
   - replace all instances of the `github.com/embp/pulumi-hivelocity` repository with the `REPOSITORY` location
...
before:
  ...
  - make tfgen
builds:
- binary: pulumi-resource-hivelocity
  dir: provider
  goarch:
  - amd64
  - arm64
  ...
  ignore: []
  ldflags:
  - -X github.com/embp/pulumi-hivelocity/provider/pkg/version.Version={{.Tag}}
  main: ./cmd/pulumi-resource-hivelocity/
  sort: asc
  use: git
release:
  disable: false
...
├── config.yaml
├── metadata.json
├── pulumi-hivelocity_darwin_amd64_v1
│   └── pulumi-resource-hivelocity
├── pulumi-hivelocity_darwin_arm64
│   └── pulumi-resource-hivelocity
├── pulumi-hivelocity_linux_amd64_v1
│   └── pulumi-resource-hivelocity
├── pulumi-hivelocity_linux_arm64
│   └── pulumi-resource-hivelocity
├── pulumi-hivelocity_windows_amd64_v1
│   └── pulumi-resource-hivelocity.exe
└── pulumi-hivelocity_windows_arm64
    └── pulumi-resource-hivelocity.exe
```
